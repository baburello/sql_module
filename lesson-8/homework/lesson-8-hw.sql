CREATE DATABASE HOMEWORK_LESSON_7
USE HOMEWORK_LESSON_7
...
--7
SELECT *, IIF(StockQuantity < 30, 'Low Stock', 'Sufficient') AS [Stock Status]
FROM Products
...
SELECT MIN(Quantity) AS [MIN QUANTITY],MAX(Quantity) AS [MAX QUANTITY] FROM Orders
select * from Orders
...
--13
SELECT YEAR(OrderDate) AS [ORDER DATE],
...
SELECT ProductID, SUM(SaleAmount) AS [TOTAL SALES] FROM Sales
GROUP BY ProductID
...
SELECT TOP 3 CustomerID, SUM(TotalAmount) AS [TOTAL AMOUNT]
FROM Invoices
GROUP BY CustomerID
...
--22
select * from Orders
SELECT TOP 3 ProductID, SUM(TotalAmount) AS [TOTAL AMOUNT]
From Orders
GROUP BY ProductID
ORDER BY [TOTAL AMOUNT] DESC
